...
def generate():
    ...
    add_body_paragraph(doc, "Механизм логического вывода экспертной системы основан на последовательной проверке правил. Входными данными являются ответы студента на 11 вопросов: специальность, средний балл, желаемый город, предпочтительный формат работы, предпочтительная отрасль и шесть вопросов о профессиональных интересах. На основании этих ответов система формирует список навыков и интересов студента, после чего сопоставляет его профиль с фактами базы знаний.")
    add_body_paragraph(doc, "Алгоритм работы системы можно разделить на два уровня. Первый уровень выполняет обязательные проверки и исключает неподходящие варианты. Второй уровень рассчитывает степень совпадения навыков и профессиональных интересов, формирует объяснение результата и сортирует рекомендации по убыванию процента соответствия.")
    ...
    add_bullet_item(doc, "соответствие предпочтительной отрасли предприятия;")
    add_bullet_item(doc, "соответствие предпочтительного формата работы: офис, удаленная работа или любой формат.")
    add_body_paragraph(doc, "После выполнения обязательных критериев система вычисляет итоговый показатель Score. Базовые 50% начисляются за прохождение жестких фильтров, до 30% добавляется за совпадение требуемых навыков, до 20% — за совпадение профессиональных интересов.")

    # 5.1. Графовая модель
    add_heading_2(doc, "5.1. Графовая модель базы знаний")
    add_body_paragraph(doc, "Графовая модель базы знаний отражает структуру связей между объектами предметной области, фактами Prolog и правилами логического вывода. Вершинами графа являются элементы анкеты студента, факты о компаниях, факты о вакансиях, профессиональные интересы, правила проверки и конечные результаты консультации. Ребра показывают, какие элементы используются совместно при формировании рекомендации.")
    
    # Текстовое описание графа базы знаний
    add_body_paragraph(doc, "В графовой модели выделены следующие основные ветви:")
    add_bullet_item(doc, "Анкета студента — содержит 11 ответов пользователя, из которых формируются параметры профиля StudentSkills и StudentInterests.")
    add_bullet_item(doc, "Факты company/6 — описывают предприятия, отрасль, город и минимальный средний балл.")
    add_bullet_item(doc, "Факты position/6 — описывают вакансии, подходящие специальности, требуемые навыки и связь вакансии с компанией.")
    add_bullet_item(doc, "Факты position_interests/2 — связывают вакансии с профессиональными интересами.")
    add_bullet_item(doc, "Правила вывода — выполняют проверки match_specialization/2, match_gpa/2, match_location/2, match_industry/2, match_work_format/2, interest_score/3 и recommend_position/13.")
    add_bullet_item(doc, "Результат консультации — содержит ранжированный список рекомендованных вакансий и список недостающих навыков.")
    
    add_body_paragraph(doc, "Схематичное изображение графовой модели базы знаний представлено на Рисунке 1.")
    ...
    run_desc = p_img_desc.add_run("Рисунок 1 — Графовая модель базы знаний экспертной системы")
    ...
    add_body_paragraph(doc, "4. Проверка отрасли и формата работы:", bold=True)
    add_code_block(doc, "ЕСЛИ (Отрасль_Студента = Отрасль_Компании) ИЛИ (Отрасль_Студента = any), ТО Отрасль_Подходит = да.\n"
                   "ЕСЛИ (Формат_Студента = Формат_Компании) ИЛИ (Формат_Студента = any), ТО Формат_Подходит = да.")

    add_body_paragraph(doc, "5. Расчет коэффициента совпадения:", bold=True)
    add_code_block(doc, "ЕСЛИ (Специальность_Подходит = да) И (Балл_Подходит = да) И (Локация_Подходит = да) И (Отрасль_Подходит = да) И (Формат_Подходит = да),\n"
                   "ТО Вакансия_Рекомендуется = да, Базовый_Балл = 50%.\n"
                   "Балл_Навыков = 30 * (Количество_Совпавших_Навыков / Общее_Количество_Требований_Вакансии).\n"
                   "Балл_Интересов = 20 * (Количество_Совпавших_Интересов / Общее_Количество_Интересов_Студента).\n"
                   "Итоговый_Процент_Соответствия = Базовый_Балл + Балл_Навыков + Балл_Интересов.")
    add_body_paragraph(doc, "Преимущество продукционной формы заключается в прозрачности рассуждения. Каждое правило можно проверить отдельно, а результат консультации легко объяснить пользователю: если вакансия не попала в выдачу, значит, не выполнен один из обязательных критериев; если вакансия попала в выдачу, то процент соответствия показывает степень совпадения навыков и профессиональных интересов.")
    add_body_paragraph(doc, "В программной реализации продукционные правила представлены предикатами match_specialization/2, match_gpa/2, match_location/2, match_industry/2, match_work_format/2, intersect_count/3, interest_score/3, missing_skills/3 и recommend_position/13. Главный предикат recommend_position/13 объединяет все проверки и формирует итоговую рекомендацию.")
